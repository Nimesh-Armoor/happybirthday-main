...
const animationTimeline = () => {
    // Split characters for individual animations
    const textBoxChars = document.getElementsByClassName("hbd-chatbox")[0];
    const hbd = document.getElementsByClassName("wish-hbd")[0];

    textBoxChars.innerHTML = `<span>${textBoxChars.innerHTML.split("").join("</span><span>")}</span>`;
    hbd.innerHTML = `<span>${hbd.innerHTML.split("").join("</span><span>").replace(/ /g, "&nbsp;")}</span>`;
    ...
    tl.to(".container", 0.6, { visibility: "visible" })
      ...
      .to(".one", 0.7, { opacity: 0, y: 10 }, "+=1.5") // Ensure enough time for .one to fade
      .from(".three", 0.7, { opacity: 0, y: 10 })
      .to(".three", 0.5, { opacity: 0, y: 10 }, "+=1.5") // Reduced delay to overlap more

      // Slight overlap to start `.four` quickly after `.three` ends
      .from(".four", 0.5, { scale: 0.2, opacity: 0, y: -10 }, "-=0.3") // Overlap more with .three's end

      // Additional animations as needed
      .from(".fake-btn", 0.3, { scale: 0.2, opacity: 0 }, "+=1")
      .staggerTo(".hbd-chatbox span", 1.5, { visibility: "visible" }, 0.05)
      .to(".fake-btn", 0.1, { backgroundColor: "rgb(127, 206, 248)" }, "+=1")
      .to(".four", 0.7, { scale: 0.2, opacity: 0, y: -150 }, "+=1")

      // Other idea animations
